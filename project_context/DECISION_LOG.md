...
After the S1 (CLOB `/prices-history`) negative, the first candidate alternative per-side price source — local trade-print reconstruction via `Store.load_trades()` (Option A, no network) — was implemented per `SPEC_price_source_alt_trade_prints.md` (approved, spec only, precision-loss correction applied) as a coverage-only, local-only, user-run test, reusing the **exact accepted S1 Pass-1 300-condition sample** for direct comparability, and **completed with a NEGATIVE Pass-1 sampled result.**
- **Result.** On the accepted run: sample 300/300 reconstructed as the union of the accepted S1 by-condition + excluded ledgers; **248 measured-eligible**, **52 S1-invalid-window** pre-excluded + reported (the same 52 conditions the accepted S1 run classified `NO_VALID_DECISION_WINDOW_AFTER_WARMUP`, carried forward and never re-measured — so they cannot become coverage negatives regardless of what the local trades admit). `measured_by_subclass` UP_DOWN 50 / OVER_UNDER 98 / NAMED_OTHER 100 — **identical** to the accepted S1 breakdown, confirming same-sample reuse held exactly. Level-B class counts: BOTH_SIDES 124 / ONE_SIDE 65 / NEITHER 59 (sums to 248 measured). **Level-B both-sides coverage (the binding criterion) cleared 0.95 in no subclass:** UP_DOWN 13/50 = 0.26, OVER_UNDER 40/98 ≈ 0.4081632653, NAMED_OTHER 71/100 = 0.71. Verdict **`S1ALT_SOURCE_NOT_VIABLE`**. Figures reconcile: 248 + 52 = 300; measured 50 + 98 + 100 = 248; Level-B 124 + 65 + 59 = 248; per-subclass both-sides 13 + 40 + 71 = 124. Diagnostics: `token_pair_unresolved` 0, `malformed_trade_rows_total` 19, `invalid_price_rows_total` 0, `all_one_status_detected` false (genuinely mixed, not a suspicious uniform output), `level_c_validation` passed.
- **Consequence.** Local trade prints do **not** provide a usable decision-window per-side price for both sides across the sampled P0 universe either. **P1 remains BLOCKED with no `yes_price` fallback and no `1 - price` synthesis.** This is **Pass 1 sampled coverage only** — not Pass 2 full-universe coverage; the sampled negative was accepted as the finding. A later S2 build spec was gated on *favorable* S1 (or S1-ALT) coverage, which did not occur for either candidate, so **S2 does not follow** on this evidence. No Pass 2, Option B, Option C, S2, P1/P2/P3, probe, scoring, backfill, or gate change is authorized; `named_binary_probe_blocked` stays `true`.
- **Artifact-vs-finding discipline (meta).** As with S1, the verdict was only trusted **after** the measurement was made healthy. Four issues were caught and fixed **before** acceptance, each diagnosed against the authoritative accepted S1 artifacts / data contracts rather than assumed: (1) a setup/schema-stop gap where a missing/malformed required pinned input (classification contract or resolution-source parquet) could have flowed through as blank `resolved_at` for every condition → all invalid windows → a false `S1ALT_INCONCLUSIVE_NO_VALID_DECISION_WINDOW_SAMPLE` — a setup failure masquerading as a data finding — fixed with typed `STOP_CONTRACT_SCHEMA`/`STOP_RESOLUTION_SOURCE_SCHEMA` stops firing before any coverage classification; (2) a sample-reconstruction bug that rejected the accepted S1 ledgers' expected 52-id overlap (the invalid-window ids legitimately appear in both S1 ledgers) as `STOP_SAMPLE_IRREPRODUCIBLE`, fixed by reconstructing the sample as a union with the valid/invalid split read from status; (3) an excluded-reason exact-match bug that rejected the accepted S1 excluded ledger's real shape (class + parenthesized diagnostic suffix in the same `reason` field), fixed by normalizing to the leading class token before comparison while preserving the raw string for audit; (4) an `outcome_index` precision-handling bug where a bounded `{0,1}` integer slot value (`0.0`, from a real pandas `int→float64` column upcast) was wrongly evaluated under `token_id`'s strict 78-digit-identifier rule and hard-stopped as `STOP_PRECISION_LOSS`, fixed with a separate, narrowly-scoped normalization rule for `outcome_index` that never treats a domain violation as precision loss and never loosens `token_id`'s strict rule. The accepted run reconciles exactly at every level and shows a genuinely mixed Level-B distribution (not an all-one-status output), so the passing `level_c_validation` is a real corroboration.
